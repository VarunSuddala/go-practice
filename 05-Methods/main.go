...
func main() {
	...
	for {
		...
		case 1:
			...
			fmt.Scan(&password)
			account, err := makeAccount(name, email, balance, address, accountType, password)
			if err != nil {
				fmt.Println("Account creation failed:", err)
			} else {
				banks[account.AccountNumber] = account
		...
		case 2:
			...
			fmt.Scan(&password)
			Bank, exists := banks[accountNumber]
			if exists {
		...
		case 5:
			...
			fmt.Scan(&password)
			senderBank, senderExists := banks[senderAccountNumber]
			recipientBank, recipientExists := banks[recipientAccountNumber]
